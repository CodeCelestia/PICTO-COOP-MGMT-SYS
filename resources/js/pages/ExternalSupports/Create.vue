...
import { ArrowLeft, LifeBuoy, Save, X } from 'lucide-vue-next';
...
const handleBack = () => {
    if (isPerCoopRoute.value) {
        window.location.href =
            `/cooperatives/${coopIdFromUrl.value}?tab=finance&subtab=external-support`;
        return;
    }
    ...
    <FinanceShellLayout active-tab="external-supports" :hide-tabs="isFromCoopContext">
        <div class="space-y-6 p-4 sm:p-6">
            <div class="flex items-start justify-between gap-4">
                <div class="space-y-1">
                    <h1 class="text-2xl font-semibold tracking-tight text-foreground sm:text-3xl">Add External Support</h1>
                    <p class="text-sm text-muted-foreground">Record external support or assistance.</p>
                </div>
