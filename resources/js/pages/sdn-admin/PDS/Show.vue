...
<template>
    <Head :title="fullName" />
    <AppLayout>
        <div class="flex flex-1 flex-col gap-6 max-w-5xl mx-auto w-full p-6">
            <!-- Gradient header -->
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <div class="flex items-center gap-2 mt-1.5">
                                ...
                                <span v-if="pds.office" class="inline-flex items-center rounded-full px-2 py-0.5 text-xs font-semibold bg-white/20 text-white">{{ pds.office.name }}</span>
                            </div>
                        </div>
                    </div>
                    <div class="flex gap-2">
                        <Button class="bg-white/20 hover:bg-white/30 text-white border border-white/30" @click="router.visit(`/sdn-admin/pds`)">Back</Button>
                        <Button class="bg-white/20 hover:bg-white/30 text-white border border-white/30 gap-2" @click="router.visit(`/sdn-admin/pds/${pds.id}/edit`)">
            ...
            <div class="rounded-xl border border-slate-200 bg-white shadow-sm p-5 space-y-4">
                ...
                <div class="pt-2 border-t space-y-2 text-sm">
                    <div>
                        <p class="text-xs text-muted-foreground font-medium">Residential Address</p>
                    ...
                    <div>
                        <p class="text-xs text-muted-foreground font-medium">Permanent Address</p>
            ...
            <div v-if="pds.education && pds.education.some((e: any) => e.school_name)" class="rounded-xl border border-slate-200 bg-white shadow-sm p-5">
                ...
                <div class="overflow-x-auto rounded-lg border">
                    <table class="min-w-full text-xs">
                        <thead class="bg-muted/40 border-b"><tr>
                            ...
                            <th class="px-3 py-2 text-left">Yr. Grad</th>
                            <th class="px-3 py-2 text-left">Honors</th>
                        </tr></thead>
                        <tbody class="divide-y">
                            <tr v-for="row in pds.education.filter((e: any) => e.school_name)" :key="row.level" class="hover:bg-muted/10">
            ...
            <div v-if="pds.eligibilities && pds.eligibilities.length" class="rounded-xl border border-slate-200 bg-white shadow-sm p-5">
                ...
                <div class="overflow-x-auto rounded-lg border">
                    <table class="min-w-full text-xs">
                        <thead class="bg-muted/40 border-b"><tr>
                            <th class="px-3 py-2 text-left">Examination</th>
                            <th class="px-3 py-2 text-left">Rating</th>
                            <th class="px-3 py-2 text-left">Date</th>
                            <th class="px-3 py-2 text-left">Place</th>
                            <th class="px-3 py-2 text-left">License No.</th>
                            <th class="px-3 py-2 text-left">Validity</th>
                        </tr></thead>
                        <tbody class="divide-y">
                            <tr v-for="(e,i) in pds.eligibilities" :key="i" class="hover:bg-muted/10">
                                <td class="px-3 py-2">{{ e.exam_name }}</td>
                                <td class="px-3 py-2">{{ e.rating||"—" }}</td>
                                <td class="px-3 py-2">{{ e.exam_date||"—" }}</td>
                                <td class="px-3 py-2">{{ e.exam_place||"—" }}</td>
                                <td class="px-3 py-2">{{ e.license_no||"—" }}</td>
                                <td class="px-3 py-2">{{ e.validity_date||"—" }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                ...
                <div class="overflow-x-auto rounded-lg border">
                    <table class="min-w-full text-xs">
                        <thead class="bg-muted/40 border-b"><tr>
                            ...
                            <th class="px-3 py-2 text-left">SG</th>
                            <th class="px-3 py-2 text-left">Status</th>
                            <th class="px-3 py-2 text-center">Gov</th>
                        </tr></thead>
                        <tbody class="divide-y">
                            <tr v-for="(w,i) in pds.work_experiences" :key="i" class="hover:bg-muted/10">
                                ...
                                <td class="px-3 py-2">{{ w.position_title }}</td>
                                <td class="px-3 py-2">{{ w.department }}</td>
                                <td class="px-3 py-2">{{ w.monthly_salary ? "₱"+Number(w.monthly_salary).toLocaleString() : "—" }}</td>
                                <td class="px-3 py-2">{{ w.salary_grade||"—" }}</td>
                                <td class="px-3 py-2">{{ w.appointment_status||"—" }}</td>
                        ...
                        <table class="min-w-full text-xs">
                            ...
                            <tbody class="divide-y">
                                <tr v-for="(v,i) in pds.voluntary_works" :key="i" class="hover:bg-muted/10">
                                    ...
                                    <td class="px-3 py-2">{{ v.hours }}</td>
                                    <td class="px-3 py-2">{{ v.position }}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
