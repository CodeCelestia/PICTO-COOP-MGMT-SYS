...
const visibleRecords = computed(() => props.records.data);
...
const {
    ...
    toggleAll,
    toggleOne,
} = useBulkSelection(visibleRecords);

const bulkDeleteRecords = async () => {
    if (!selectedCount.value || !canBulkDelete.value) return;

    const confirmed = await confirmAction({
        title: 'Delete selected financial records?',
        text: `Delete ${selectedCount.value} selected financial record(s)? This action cannot be undone.`,
    ...
    await runBulkDelete(idsToDelete, (id) => `/financial-records/${id}`);
    clearSelection();
};
</script>
            ...
            <div class="rounded-xl border border-border bg-card/95 p-4 shadow-sm sm:p-5">
                ...
                <div class="flex items-center gap-2 self-start">
                    <div v-if="canBulkDelete && selectedCount > 0" class="flex items-center gap-2 rounded-md border border-border/70 bg-muted/40 px-2 py-1">
                        ...
                        <Button size="sm" variant="destructive" class="h-8 gap-1.5" @click="bulkDeleteRecords">
                ...
                <FilterPanel
                    title="Filters"
                    description="Show financial record filters when ready."
                        ...
                        <TableRow>
                            <TableHead v-if="canBulkDelete" class="w-12">
                                <Checkbox
                                    ...
                                    aria-label="Select all financial records"
                            ...
                            <TableRow v-for="record in records.data" :key="record.id">
                                <TableCell v-if="canBulkDelete" class="w-12">
                                    <Checkbox
                                        ...
                                        :aria-label="`Select financial record ${record.period}`"
                                        @update:model-value="(checked) => toggleOne(record.id, checked)"
                                    />
                                </TableCell>
                                <TableCell>
                    ...
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <div class="flex flex-wrap gap-2" aria-label="Financial records pagination">
                            <Button
                                ...
                                @click="router.get('/financial-records', { ...filters, page }, { preserveScroll: true, preserveState: true })"
                            >
                                {{ page }}
                            </Button>
                        </div>
