...
const outcomesAttachments = computed(() => form.outcomes_attachment
...
const updateOutcomesAttachment = (event: Event) => {
    ...
    for (const file of files) {
        if (form.outcomes_attachment.length >= MAX_OUTCOMES_FILES) {
            break;
        }
    ...
    if (files.length > 0 && form.outcomes_attachment.length > 0) notifySuccess('Outcomes attachment(s) added.');
};
...
const showImageTypeError = (fileName: string) => {
    notifyError(`"${fileName}" is not a supported image format. Please use JPG, PNG, GIF, or WEBP.`);
};
...
const isValidImageType = (file: File): boolean => {
    ...
    if (!allowedTypes.includes(file.type)) {
        Swal.fire({
            ...
            html: `
                <p>"${file.name}" is not a supported image format.</p>
                ...
                <Card class="border-border/80 bg-card/95 shadow-sm">
                    ...
                    <CardContent class="space-y-4 pt-0">
                        <div ref="cooperativeSectionRef">
                            <Label for="coop_picker">Cooperatives <span class="text-red-500">*</span></Label>
                            <Button
                                id="coop_picker"
                                type="button"
                                variant="outline"
