...
import { AlertCircle, ArrowLeft, Save, Building2, ReceiptText, TrendingUp } from 'lucide-vue-next';
        ...
        <div class="space-y-6">
            <Card>
                <CardContent class="flex items-center justify-between gap-4 py-4">
                    <div class="flex min-w-0 items-center gap-3">
                        ...
                        <div class="min-w-0">
                            <h1 class="text-xl font-semibold tracking-tight text-foreground sm:text-2xl">Add Financial Record</h1>
                            <p class="mt-1 text-sm text-muted-foreground">Record cooperative financial data with a structured ledger entry.</p>
                        </div>
                    </div>
            ...
            <Card v-if="!isCoopContext">
                <CardHeader>
                    ...
                    <CardDescription>Select the cooperative for this record</CardDescription>
                </CardHeader>
                <CardContent class="space-y-4">
                    <div class="space-y-2">
                        <Label for="coop_id">Cooperative <span class="text-red-500 ml-0.5">*</span></Label>
                        <Select v-model="form.coop_id" :disabled="isCoopAdmin" @update:modelValue="clearError('coop_id')">
            ...
            <form @submit.prevent="submit" class="space-y-6" :class="{ 'animate-shake': showErrorShake }">
                <Card>
                    <CardHeader class="space-y-1 pb-4">
                        ...
                        <CardDescription>Enter the required ledger details and any supporting context.</CardDescription>
                    </CardHeader>
                    <CardContent class="pt-0">
                        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                            <div class="space-y-2">
                                <Label for="period">Period <span class="text-red-500 ml-0.5">*</span></Label>
                                <Input id="period" v-model="form.period" placeholder="2025 or 2025-Q1" :class="inputErrorClass('period')" @input="clearError('period')" />
                                <p v-if="form.errors.period" class="mt-1 flex items-center gap-1 text-sm text-red-500"><AlertCircle class="h-3.5 w-3.5 shrink-0" />{{ form.errors.period }}</p>
                            </div>
                            <div class="space-y-2">
                                <Label for="type">Type <span class="text-red-500 ml-0.5">*</span></Label>
                                <Select v-model="form.type" @update:modelValue="clearError('type')">
                                    <SelectTrigger id="type" :class="inputErrorClass('type')"><SelectValue placeholder="Select type" /></SelectTrigger>
                                    <SelectContent><SelectItem v-for="option in typeOptions" :key="option" :value="option">{{ option }}</SelectItem></SelectContent>
                                </Select>
                                <p v-if="form.errors.type" class="mt-1 flex items-center gap-1 text-sm text-red-500"><AlertCircle class="h-3.5 w-3.5 shrink-0" />{{ form.errors.type }}</p>
                            </div>
                            <div class="space-y-2">
                                <Label for="source">Source <span class="text-xs font-normal text-muted-foreground ml-1">(Optional)</span></Label>
                                <Input id="source" v-model="form.source" placeholder="LGU, Cooperative, Grant" :class="inputErrorClass('source')" @input="clearError('source')" />
                                <p v-if="form.errors.source" class="mt-1 flex items-center gap-1 text-sm text-red-500"><AlertCircle class="h-3.5 w-3.5 shrink-0" />{{ form.errors.source }}</p>
                            </div>
                            <div class="space-y-2">
                                <Label for="date_recorded">Date Recorded <span class="text-xs font-normal text-muted-foreground ml-1">(Optional)</span></Label>
                                <Input id="date_recorded" v-model="form.date_recorded" type="date" :class="inputErrorClass('date_recorded')" @input="clearError('date_recorded')" />
                                <p v-if="form.errors.date_recorded" class="mt-1 flex items-center gap-1 text-sm text-red-500"><AlertCircle class="h-3.5 w-3.5 shrink-0" />{{ form.errors.date_recorded }}</p>
                            </div>
                            <div class="col-span-1 space-y-2 md:col-span-2">
                                <Label for="purpose">Purpose / Description <span class="text-xs font-normal text-muted-foreground ml-1">(Optional)</span></Label>
                                <Textarea id="purpose" v-model="form.purpose" placeholder="Provide details about this financial transaction" rows="3" :class="inputErrorClass('purpose')" @input="clearError('purpose')" />
                                <p v-if="form.errors.purpose" class="mt-1 flex items-center gap-1 text-sm text-red-500"><AlertCircle class="h-3.5 w-3.5 shrink-0" />{{ form.errors.purpose }}</p>
                            </div>
                        </div>
                    </CardContent>
                </Card>
                ...
                <Card>
                    <CardHeader class="space-y-1 pb-4">
                        ...
                        <CardDescription>Optional amounts that support the ledger record.</CardDescription>
                    </CardHeader>
                    <CardContent class="pt-0">
                        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                ...
                <Card>
                    ...
                    <CardContent class="pt-0">
                        <div class="space-y-2">
                            <Label for="reference_doc">Reference Document <span class="text-xs font-normal text-muted-foreground ml-1">(Optional)</span></Label>
                            <Input id="reference_doc" v-model="form.reference_doc" placeholder="Receipt number, invoice ID, etc." :class="inputErrorClass('reference_doc')" @input="clearError('reference_doc')" />
                            <p v-if="form.errors.reference_doc" class="mt-1 flex items-center gap-1 text-sm text-red-500"><AlertCircle class="h-3.5 w-3.5 shrink-0" />{{ form.errors.reference_doc }}</p>
                        </div>
                    </CardContent>
                </Card>
                ...
                <div class="flex justify-end gap-3">
                    ...
                    <Button type="submit" :disabled="form.processing || !canCreate" class="gap-2 bg-foreground text-background hover:bg-foreground/90"><Save class="h-4 w-4" />Save Record</Button>
                </div>
            </form>
        </div>
    </FinanceShellLayout>
        ...
        <div class="space-y-6 p-4 sm:p-6">
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>Enter the financial record information below</CardDescription>
                </CardHeader>
                <CardContent>
