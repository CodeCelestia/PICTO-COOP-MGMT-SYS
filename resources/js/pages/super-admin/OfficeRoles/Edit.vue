...
import { update as officeRolesUpdate } from '@/routes/super-admin/office-roles';
...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Edit', href: `/super-admin/office-roles/${props.role.id}/edit` },
];
...
const submit = () => {
    form.put(officeRolesUpdate(props.role.id).url, {
        onSuccess: () => swalSuccess('Role Updated!', `"${form.display_name}" has been updated successfully.`),
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex flex-1 flex-col gap-6 max-w-3xl mx-auto w-full">
            ...
            <div class="rounded-2xl bg-linear-to-r from-purple-600 to-pink-600 px-6 py-5 text-white shadow-lg">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h1 class="text-xl font-bold">Edit Office Role</h1>
                            <p class="text-sm text-purple-200">Update role details for {{ role.display_name }}</p>
                        </div>
                    </div>
            ...
            <form @submit.prevent="submit" class="space-y-5">
                ...
                <!-- System Role Warning -->
                <div v-if="role.is_system" class="rounded-xl border border-amber-200 bg-amber-50 p-4">
                    <div class="flex items-start gap-3">
                        <Shield class="h-5 w-5 text-amber-600 mt-0.5" />
                        <div>
                            <p class="text-sm font-semibold text-amber-900">System Role</p>
                            <p class="text-xs text-amber-700">This is a system role. Changes should be made carefully.</p>
                        </div>
                    </div>
                </div>
